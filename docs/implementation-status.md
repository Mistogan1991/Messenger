...
| Search (global / messages) | 🟡 | **Message search** across the user's chats (`GET /api/messages/search?q=`, case-insensitive content match, member-scoped). _(M4)_ User/chat search still pending |
...
| `MessagesController` | send, edit, delete, reply, forward, read, reaction add/remove, attachment, **GET chat/{chatId}** (paged), **GET {messageId}**, **GET search** | 🟡 | pin/unpin message |
