...
| A5 | ~~Soft-delete filter disabled~~ | ✅ Done (M0) | Global `HasQueryFilter` (`e => e.DeletedAtUtc == null`) re-enabled in `OnModelCreating` for every `SoftDeletableEntity<Guid>`. All domain entities are soft-deletable (only `OutboxMessage` isn't), so there is no principal/dependent filter mismatch. Use `IgnoreQueryFilters()` to read deleted rows. No schema change. | — |
