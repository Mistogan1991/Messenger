...
|---|---|---|---|---|
| A1 | ~~Domain events not dispatched~~ | ✅ Done (M0) | Events now raised in `Message`/`Chat` and dispatched from `ApplicationDbContext.SaveChangesAsync`. Domain stays MediatR-free: aggregates raise pure `IDomainEvent`; `DomainEventDispatcher` wraps each in `DomainEventNotification<T>` (`INotification`) and publishes via `IPublisher`. Dispatch is **post-commit** (after `SaveChanges`); a transactional outbox is still the follow-up (A4). | — |
...
| A4 | No transactional consistency / outbox | 🟠 | Cross-aggregate operations and future integration events aren't atomic; no reliable publish. | Add `TransactionBehavior` and a transactional **outbox** before introducing RabbitMQ. |
